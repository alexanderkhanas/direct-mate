...
let ReservationsService = ReservationsService_1 = class ReservationsService {
    ...
    async expireStale() {
        const stale = await this.reservationRepo.find({
            where: { status: shared_1.ReservationStatus.Active, expiresAt: (0, typeorm_2.LessThan)(new Date()) },
        });
        if (stale.length === 0)
            return 0;
        for (const reservation of stale) {
            await this.reservationRepo.update(reservation.id, { status: shared_1.ReservationStatus.Expired });
            const stock = await this.stockRepo.findOne({ where: { variantId: reservation.variantId } });
            if (stock) {
                await this.stockRepo.update(stock.id, {
                    reservedQty: Math.max(0, stock.reservedQty - reservation.qty),
...
], ReservationsService.prototype, "expireStale", null);
